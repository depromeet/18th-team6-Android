...
class AgentViewModel internal constructor(
    ...
    fun createAgent(name: String, description: String, type: AgentType) = intent {
        viewModelScope.launch {
            agentRepository.createAgent(name, description, type).onSuccess {
    ...
    fun deleteAgent(id: Int) = intent {
        viewModelScope.launch {
            agentRepository.deleteAgent(id).onSuccess {
                // 성공에 대한 동작
            }.onFailure { e ->
                postSideEffect(AgentSideEffect.OnError(e))
            }
    ...
    fun patchAgent(id: Int, name: String, description: String, type: AgentType) = intent {
        viewModelScope.launch {
            agentRepository.patchAgent(PatchAgentParams(
                id = id,
                name = name,
...
sealed interface AgentSideEffect {
    data class OnAgentClick(val agent: Agent) : AgentSideEffect
    data class OnAgentLongClick(val agent: Agent) : AgentSideEffect
    data object OnMenuClick : AgentSideEffect
    data class ShowSnackbar(val message: String) : AgentSideEffect
    data class OnError(val error: Throwable) : AgentSideEffect
}
